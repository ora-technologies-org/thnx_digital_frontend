...
export const activityLogQueryKeys = {
  ...
  logs: (filters: ActivityLogFilters) => [...activityLogQueryKeys.all, 'list', filters] as const,
  stats: (merchantId?: string) => [...activityLogQueryKeys.all, 'stats', merchantId] as const,
  timeline: (resourceType: string, resourceId: string) =>
    [...activityLogQueryKeys.all, 'timeline', resourceType, resourceId] as const,
};
...
export const useActivityLogs = (initialFilters: ActivityLogFilters = {}) => {
  ...
  const addRealtimeLog = useCallback((log: ActivityLog) => {
    ...
    if (filters.search && !log.description.toLowerCase().includes(filters.search.toLowerCase())) return;
  ...
  const refresh = useCallback(() => {
    setRealtimeLogs([]);
    setNewLogIds(new Set());
    queryClient.invalidateQueries({ queryKey: activityLogQueryKeys.logs(filters) });
  }, [queryClient, filters]);
...
export const useResourceTimeline = (resourceType: string, resourceId: string) => {
  return useQuery({
